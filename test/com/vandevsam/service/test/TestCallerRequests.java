...
public class TestCallerRequests {
	...
	public void testCallRequests() {
		...
		boolean result = ServiceMonitor.registerCaller(localHost, httpPort,
				callerID);
		// should return false since the service hasn't started yet and hence
		// it's not
		...
		// allow for time until the service is established before requesting to
		// monitor
		// the service
		...
		ServiceMonitor service2 = new ServiceMonitor(remoteHost, httpPort,
				frequency);
		new Thread(service2, "remotehost").start();

		waitup();

		assertTrue(ServiceMonitor
				.registerCaller(remoteHost, httpPort, callerID));
		// should return false, since this caller has already requested
		// successfully this service
		assertFalse(ServiceMonitor
				.registerCaller(localHost, httpPort, callerID));
