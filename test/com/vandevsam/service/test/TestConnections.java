...
public class TestConnections {
	...
	public void testConn() {
		...
		ServiceMonitor service2 = new ServiceMonitor(localHost, mysqlPortBad, frequency);
		new Thread(service2,"mysql").start();
		
		waitup();
		
		//this will return false since the service never connected and the callers 
		//was not instantiated as a result
		assertFalse(ServiceMonitor.registerCaller(localHost, mysqlPortBad, callerID));
